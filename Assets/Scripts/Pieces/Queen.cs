    ...
    public List<Coordinate.Point> GetAvailableSquares() {
        ...
        while (canGo) {
            if (board.CanPlace(tmpCoordinate)) {
                ...
                tmpCoordinate = Coordinate.AddPoints(tmpCoordinate, new Coordinate.Point{x=1, y=0}); 
            } else {
                canGo = false;
            }
        }
        ...
        while (canGo) {
            if (board.CanPlace(tmpCoordinate)) {
                listToReturn.Add(tmpCoordinate);
                tmpCoordinate = Coordinate.AddPoints(tmpCoordinate, new Coordinate.Point{x=0, y=1}); 
            } else {
                canGo = false;
            }
        }
